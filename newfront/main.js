  ...
  if(path.startsWith('/api/users')){
    ...
    if(path === '/api/users/delete' && options.method==='POST'){
      ...
      if(idx>-1){
        ...
        if(u.role==='مشرف أعلى') return {success:false,error:'لا يمكن حذف المشرف الأعلى'};
...
document.getElementById('users-btn-new').onclick = () => showPage('section-add-user');

document.getElementById('form-add-user').onsubmit = e => {
  e.preventDefault();
  const f = e.target;
  const newUser = {
    id: (mockData.users.length? mockData.users[mockData.users.length-1].id : 0) + 1,
...
document.querySelector('#users-tbl tbody').addEventListener('click', e => {
  ...
  if(t.classList.contains('users-delete')){
    ...
    const idx = mockData.users.findIndex(x=>x.id===id);
    if(idx>-1){
      if(mockData.users[idx].role==='مشرف أعلى') { alert('لا يمكن حذف المشرف الأعلى'); return; }
      mockData.users.splice(idx,1);
      loadUsers();
    }
  }
...
document.getElementById('form-edit-user').onsubmit = e => {
  ...
  if(u){
    ...
    if(f.password.value){ /* password ignored in mock */ }
    u.role = f.role.value;
    loadUsers();
    showPage('section-users-list');
...
function loadFinancialYear(){
  ...
  document.getElementById('fy-status').innerText = 'الحالة: ' + (y.status || 'غير مفعلة');
}
...
document.getElementById('j-refresh').onclick = async () => {
  ...
  rows.forEach(r => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${r.id||''}</td><td>${r.date||''}</td><td>${r.desc||''}</td><td>${r.debit||0}</td><td>${r.credit||0}</td><td><button class="small-btn">عرض</button></td>`;
    tbody.appendChild(tr);
  });
};
...
function populateFaculties(){
  ...
  if(!sel) return;
  sel.innerHTML = (mockData.faculties || []).map(f=>`<option value="${f.id}">${f.name}</option>`).join('');
}
document.getElementById('ef-view').onclick = () => {
  ...
  document.getElementById('ef-summary').innerText = `مجموع الإيرادات للكلية: ${sum.toFixed(2)}`;
};

// Student revenues
document.getElementById('sr-refresh').onclick = () => {
  const results = mockData.revenues || [];
  document.getElementById('sr-results').innerText = JSON.stringify(results, null, 2);
};

// Account-specific reports / movements
function populateAccountSelectors(){
  const opts = (accounts || []).map(a=>`<option value="${a.code}">${a.name}</option>`).join('');
  const ar = document.getElementById('ar-account'); if(ar) ar.innerHTML = opts;
  const am = document.getElementById('am-account'); if(am) am.innerHTML = opts;
}
document.getElementById('ar-view').onclick = () => {
  ...
  const rows = entries.flatMap(en => en.transactions.filter(t=>t.account===code).map(t=>({date:en.entryDate, type:t.type, amount:t.amount, desc:en.description})));
  document.getElementById('ar-report').innerText = JSON.stringify(rows.filter(r=> (from? r.date>=from:true) && (to? r.date<=to:true) ), null, 2);
};
document.getElementById('am-view').onclick = () => {
  const code = document.getElementById('am-account').value;
  const rows = entries.filter(en => en.transactions.some(t=>t.account===code));
  document.getElementById('am-movements').innerText = JSON.stringify(rows, null, 2);
};
...
document.getElementById('s-search').onclick = () => {
  ...
  if(!type || type==='account'){
    const found = (accounts || []).filter(a => a.code.includes(q) || a.name.toLowerCase().includes(q));
    out += 'Accounts:\n' + JSON.stringify(found, null, 2) + '\n';
  }
  if(!type || type==='entry'){
    const found = (entries || []).filter(en => en.description.toLowerCase().includes(q) || en.entryDate.includes(q) || en.transactions.some(t=>t.account.includes(q)) );
    out += 'Entries:\n' + JSON.stringify(found, null, 2);
  }
...
try{
  ...
  if(localStorage.getItem('loggedIn')){ if(logoutBtn) logoutBtn.style.display = 'inline-block'; }
